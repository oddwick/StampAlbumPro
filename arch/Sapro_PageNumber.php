...
class Sapro_PageNumber extends Nerb_PageNumber {
		...
		public function __toString(){
			...
			for($i = $startPage; $i < $this->_params['truncate'] + $startPage && $i <=  $this->_totalPages; $i++){
				if($i == $this->_currentPage){
					...
					$string .= "<span class='".$this->_params['currentPageClass']."'>".str_replace("*", $i, $this->_params['currentPageLocator'])."</span>";
				} else {
			...
			if( $this->_currentPage - $mid > 2){ 
				$string = $this->_jslink(1)."&nbsp;&hellip;&nbsp;".$string;
			}
			
			// adds a perminant link to the last page
			if(  $this->_currentPage < $this->_totalPages - $mid - 1){ 
				$string .= "&hellip;&nbsp;".$this->_jslink($this->_totalPages);
			}
			
			
			// add a next link at the end of page numbers
			if( $this->_currentPage < $this->_totalPages ){
				$string .= "&nbsp;&nbsp;&nbsp;".$this->_jslink($this->_currentPage + 1, "Next");
			}
			
			return "<span class='".$this->_params['baseClass']."'>".$string."</span>";
		} // end function -----------------------------------------------------------------------------------------------------------------------------------------------
		...
		*/
		protected function _jslink($page, $text = NULL){
			...
			$click = str_replace('<year>', $this->_link, $this->_params['onClick']);
			$click = str_replace('<page>', $page, $click);
			
			$string = "<a href='javascript:void(0);' ".($this->_params['anchorClass']?'class="'.$this->_params['anchorClass'].'"':NULL)." ".($this->_params['onClick']?"onclick=\"".$click."\"":NULL).">".($text?$text:$page)."</a>";
			return $string;
		} // end function -----------------------------------------------------------------------------------------------------------------------------------------------
